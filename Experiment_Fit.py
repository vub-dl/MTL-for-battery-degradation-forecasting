...
class Fit_MTL_Optimization():
    ...
    def __init__(self, configuration):
        ...
        elif self.config["Task_Weighting_strategy"]==AlgType.IMTL.value:
            self.dynamic_alg_Type=IMTL(self.Learning_Alg)
                ...
                if self.Learning_Alg.dynamic_alg == AlgType.SLGrad:
                    ...
                    for ti in range(0, self.config["Number_of_Tasks"]):
                        ...
                        if self.Learning_Alg.dynamic_alg== AlgType.Olaux or self.Learning_Alg.dynamic_alg==AlgType.PCGrad:
                          print(self.dynamic_alg_Type.dot_prod)
        ...
        for epoch in range(0, Epoch):
            ...
            for task1, task2 in traindataloader:
                ...
                for ti in range(0, self.config["Number_of_Tasks"]):
                    ...
                    elif self.Learning_Alg.dynamic_alg== AlgType.Olaux:
                      print(self.Learning_Alg.dot_prod)
                      Task_weights=self.Learning_Alg.dot_prod
                      history["Gradient Dot"]=Task_weights
                    else:
                        history["Task_weight" + str(ti)] = Task_Weights[ti]  # save task weights
                        # save all kind of losses and metrics
