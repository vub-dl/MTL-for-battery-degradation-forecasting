...
class Seq2SeqMTL(Backbone):
    ...
    def __init__(self, seq_length=384, feature_num=2, hidden_node_dim=64, hidden_vec_size=128, shared_layers=4, task_layers=4, ntask=2, MIT=False):
        self.MIT=MIT
    ...
    def forward(self, x, task=-1, val=False, test=False, pack_tensors=False):
        ...
        if val == True:
            if self.MIT==False: #for aachen this should be dim 82
                self.sequence_length_out = 82
            else:
                self.sequence_length_out=213
        elif test == True:
            if self.MIT==False:
                self.sequence_length_out = 85
            else:
                self.sequence_length_out=215
        else:
            if self.MIT==False:
                self.sequence_length_out = 86
            else:
                self.sequence_length_out=310
