...
class MaskedMae(AbsLoss):
    ...
    def compute_loss(self, pred, gt):
        mask = torch.ne(gt, torch.tensor(0))  # Computes element-wise inequality. Returns true for non-padded elements
        masked_AE = torch.abs(torch.masked_select((gt - pred),
                                              mask))  # applies mask to the diff between ytrue and ypred. those with "false" in amsk are ignored because only the non-padded things should matter and are taken to compute abs from
        if self.reduction=='none':
          masked_MAE=masked_AE
        else:                                     
          masked_MAE = torch.sum(masked_AE) / torch.sum(mask)
        return masked_MAE

class MaskedMape(AbsLoss):

    def __init__(self, reduction='mean'):
        self.reduction=reduction

    def compute_loss(self, pred, gt):
        mask=torch.ne(gt, torch.tensor(0)) #idem mae
        masked_APE=torch.abs((torch.masked_select((gt-pred), mask))/torch.masked_select(gt,mask))*100 #idem but then with definition mape
        masked_MAPE=torch.sum(masked_APE)/torch.sum(mask)
        return masked_MAPE
